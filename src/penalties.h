...
//! Ohlmann-Thomas compressed annealing functionality.
...
public:
  ...
  */
  ...
  Compressed(const Compressed &pfun)
      : _expPower(pfun._expPower), _pressureCap(pfun._pressureCap),
...
class Simulated {
public:
  /*! Like compressed annealing, simulated annealing uses doubles for its
   * lambda. */
  typedef double ReturnType;
  constexpr static double defaultReturnTypeValue = 0.0;

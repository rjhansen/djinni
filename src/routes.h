...
class Matrix
{
  public:
    ...
    const Matrix<T, N-1>& operator[](const uint32_t n) const { return _matrix.at(n);}
    Matrix<T, N-1>& operator[](const uint32_t n) { return _matrix.at(n); }
#else
    const Matrix<T, N-1>& operator[](const uint32_t n) const { return _matrix[n]; }
    Matrix<T, N-1>& operator[](const uint32_t n) { return _matrix[n]; }
#endif
    uint32_t dimensions() const { return Matrix<T,N>::DIMENSIONS; }
    void reset()
    {
      for (uint32_t i = 0 ; i < _matrix.size() ; i += 1) _matrix[i].reset();
    }
    ...
    public:
        ...
        void generateNeighbor(TSPRoute& neighbor)
        {
            ...
            else 
            {
                ...
                neighbor._firstpenalty =
                    static_cast<uint32_t>(_penaltysum[secondswitch]);
                std::copy(_arrivaltime.begin(),
